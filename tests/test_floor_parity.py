...
governed wiki container, the shape FILE its ldp:constrainedBy points to is the
file that DEFINES a NodeShape reachable from that container's ShapeTree.
...
def _shape_iri_to_file() -> dict[str, str]:
    """Map each wiki:*Shape NodeShape IRI -> the shape filename that declares it."""
    out: dict[str, str] = {}
...
def test_constrainedby_matches_shapetree_shape(ctr):
    ...
    cb_file = Path(str(cb[0])).name

    # The shape the constrainedBy file DEFINES (its wiki:*Shape NodeShape).
    defined = {iri for iri, fname in SHAPE_FILE.items() if fname == cb_file}
    assert defined, f"{ctr}: constrainedBy file {cb_file} declares no wiki:*Shape"
    ...
    # Parity: the constrainedBy shape must be one the tree expects for this container.
    # (concepts/ reaches BOTH Concept + Source; constrainedBy names the primary, Concept.)
    overlap = defined & expected
    assert overlap, (
        f"{ctr}: ldp:constrainedBy -> {cb_file} (defines {defined}) is NOT among the "
        f"ShapeTree-expected shapes {expected} — floor/interop contract drift"
    )
