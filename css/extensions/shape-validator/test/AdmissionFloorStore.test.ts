...
// --- pod:notice marker guard (Task 10) -------------------------------------
// Bodies that contain the server-managed <!-- pod:notice marker must be
// rejected with a 422 on BOTH write paths (PUT setRepresentation + POST
// addResource). The guard must fire even when the projector returns null
// (ungoverned body) so it sits BEFORE projection. A body that mentions the
// text "pod:notice" WITHOUT the HTML-comment prefix must NOT be rejected.

describe("AdmissionFloorStore — pod:notice marker guard", () => {
  it("setRepresentation 422-rejects a body containing the server-managed pod:notice marker", async () => {
    ...
    await expect(
      store.setRepresentation({ path: RESOURCE }, md(RESOURCE, body)),
  ...
  it("setRepresentation does NOT reject a body with the plain text 'pod:notice' (no HTML-comment prefix)", async () => {
    validateMock.mockResolvedValue({ conforms: true });
    const body = "---\ntype: Concept\n---\n# X\n\nSee pod:notice for details.\n";
    const { identifierStrategy, auxiliaryStrategy } = makeStrategies();
    const projector = makeProjector(conceptProjection());
    const source = makeSource();
    const store = new AdmissionFloorStore(source as any, identifierStrategy, auxiliaryStrategy, projector as any);
  ...
  it("addResource 422-rejects a body containing the server-managed pod:notice marker", async () => {
    const body = "---\ntype: Concept\n---\n# X\n\n<!-- pod:notice — copied --> stale\n";
    const { identifierStrategy, auxiliaryStrategy } = makeStrategies();
    const projector = makeProjector(conceptProjection());
    const source = makeSource();
    ...
    await expect(
      store.addResource({ path: CONTAINER }, md(RESOURCE, body)),
    ).rejects.toThrow(/pod:notice|server-managed/);

    // Must not commit before rejecting (unlike normal non-conforming POST which commits
    // then rolls back — the marker guard fires before addResource reaches the backend).
    expect(source.addResource).not.toHaveBeenCalled();
  });

  it("addResource does NOT reject a body with the plain text 'pod:notice' (no HTML-comment prefix)", async () => {
    validateMock.mockResolvedValue({ conforms: true });
    const body = "---\ntype: Concept\n---\n# X\n\nSee pod:notice for details.\n";
    const { identifierStrategy, auxiliaryStrategy } = makeStrategies();
    const projector = makeProjector(conceptProjection());
