...
This was gated on D82: before provenance-scoped projection, a later body rewrite would strip
agent-added `.meta` triples, so a rationale written into `.meta` would not survive editing.
**D116 (PSP) dissolved that gate** — subtraction-based re-projection (`.meta − f(old) ∪ f(new)`)
makes agent `.meta` enrichment survive body rewrites by construction. The contract is now buildable.
...
### 3. Authoring surface — Option B (direct `.meta` Turtle, Turtle-lane parity)

At crystallize the agent already composes destination `.meta` Turtle (it PATCHes
`prov:wasDerivedFrom` today, per the crystallize affordance procedure). It adds one triple:

```turtle
<#this> mem:rationale "Crystallized at Chuck's request to document the two-hierarchy
  addressing rule; concluded RDFS-subsumption and SKOS-broader are distinct axes after
  reconciling D105/D106 against the cold-probe misread." .
```

The floor validates that `.meta` against the durable shape — **identical to the contact-card
contract already shipped.** No new projection grammar. Semantically honest: rationale is
write-governance metadata, not note prose. It is exactly the agent-`.meta`-enrichment case
PSP was built to make survive body rewrites — clean gate-lift narrative.

Trade accepted: rationale is not visible when you `curl` the raw `.md` body (it lives in `.meta`,
which the auditor reads anyway). A markdown-authored / frontmatter-projected surface was considered
and rejected as new mechanism for metadata that does not belong in prose.
...
| Durable seed `.meta` (6 seeds) | Add a real `mem:rationale` to `concepts/{biology,photosynthesis,two-hierarchy-memory-addressing,how-identifiers-work,how-wiki-memory-works}` + `people/marie-curie` so `make reset` admission passes. `synthesis/index` is a derived view — exempt. |
| Crystallize affordance / template (`overlays/wiki-memory/affordances/crystallize.ttl`, any concept template) | Add a `mem:rationale` slot to the `.meta` composition step + procedure prose so agents produce it by default. |
...
| Tests | New: durable write / crystallize without `mem:rationale` → laden 422; with → 201 + `mem:rationale` in `.meta`. Existing: `make reset` green; `make audit` 0 ERROR. |

## Data flow

1. Agent drafts in `/vault/wiki/working/{slug}.md` (permissive — no rationale required, D73).
2. Agent invokes crystallize: composes destination body + `.meta` Turtle including
   `<#this> mem:rationale "…"`; PUT body, PATCH `.meta`.
3. Floor loads the destination container's named shapes (per-type durable shape), dispatches by
   `rdf:type`, validates the projected+authored `.meta`. Missing/empty `mem:rationale` → 422 with
   the laden `sh:message`; the laden `sh:agentInstruction` was already readable pre-write.
...
6. A later body rewrite re-projects via PSP subtraction; the agent-authored `mem:rationale`
   survives by construction (D116).
...
- Markdown-body / frontmatter authoring surface for rationale (rejected, §3).
...
- Re-rationale-on-edit (the contract attaches at crystallization, not per subsequent edit).
