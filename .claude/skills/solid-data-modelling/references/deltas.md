...
**Authoritative artifact**: `shapes/wiki-memory-l3/{page,source,person,procedure,working,resource}.shacl.ttl`. See sibling skill `solid-wiki-memory-l3` (when created in Task 9).
...
**Authoritative artifact**: All shapes in `shapes/wiki-memory-l3/` use `sh:targetClass` against `wiki:Concept`, `wiki:Source`, etc., rather than container-path targeting.
