...
**D113/D114 (view layer + read-path authority, 2026-06-07) + D115 (SP2: consumable pod, 2026-06-12)** — view layer + SP2 sprint. D113: PROF views, ViewAssembler, Person cross-cutting demonstrator, `sub:` vocabulary. D114: trailer removed, `?_profile=fused` substrate-wide, view-authority contract. D115: SAI owner-side surfacing, `st:Description` agent-facing consumption hints, derived index views (`sub:ContainerIndex`), follow-ons deferred (markdown-lane write contract, prov:agent auth-gated). See "View layer (D113)" + "SP2: the consumable pod (D115)" sections below. **D80 re-cut (2026-06-12 SP2-T6):** hub/breadcrumb affordances revised to pointers at served views (queries removed). **D86 split verdict (2026-06-12 SP2-T7/T8):** HINT half validated + kept; SELECTION half retired (configured-client never arrived).
...
D3: Comunica for Pod SPARQL — client-side SPARQL federation over LDP; no data duplication **[2026-06-10 annotation: the principle as practiced is "the Pod hosts no SPARQL *HTTP endpoint*" — D113's ViewAssembler executes declared CONSTRUCT queries server-side via embedded Comunica. Server-side *execution of substrate-declared queries* is in; serving an open query endpoint stays out.]**
...
D12: CoreProfile conformance — Pod declares `dct:conformsTo fabric:CoreProfile, fabric:SolidPodProfile` **[2026-06-10 annotation: LIVE but NOT DEREFERENCEABLE — still emitted by `void-description.json` + the base `.meta` template, and `fabric:SolidPodProfile` IS grounded repo-side (`ontology/solid-pod-profile.ttl`, full PROF profile; CoreProfile canonical in cogitarelink-fabric). The gap is two-fold: the w3id.org/cogitarelink/fabric# redirect was never registered (RQ-Harness-1) and the ground copy isn't served from the Pod (D23 deployment gap), so D49's dereference requirement fails for agents. Disposition — keep-or-drop is a Layer-0 re-cut decision: either register w3id/serve the ground copy, or drop the declaration. Do not treat as settled.]**
...
D15: VoID feature flags — distinguish Pod-type (LDP browse) from triplestore-type fabric nodes **[2026-06-10 annotation: same non-dereferenceability as D12 (see its annotation) — `void:feature fabric:LDPBrowse` is live in the storage description, grounded repo-side, unregistered at w3id, unserved from the Pod. No fabric federation consumer has ever read it. Keep-or-drop → Layer-0 re-cut decision.]**
...
D21: Content integrity via `digestMultibase` — SHA-256 hashes in `.meta`; re-computed on TRS events **[2026-06-10 annotation: PARTIAL — digest emission exists only in the importer path (`scripts/lib/rdf_gen.py:130`); the "re-computed on TRS events" half was never built (D17 TRS superseded by D56; no recompute listener exists). Annotate as importer-side only.]**
...
D23: TBox cache in `/ontology/` container — local ontology stubs for offline/constrained queries **[2026-06-10 annotation, corrected same day: the cache IS deployed — docker-compose mounts `./ontology:/ontology` and `pod_setup.upload_ontology()` PUTs every `*.ttl` to `/vault/ontology/`. Residual gaps only: (a) non-`.ttl` cache files (e.g. `did-v1.context.jsonld`) aren't uploaded; (b) nothing links a canonical external IRI (e.g. `idot:`) to its served cache copy, so an agent can't *find* the ground copy when the canonical source is unreachable — a discoverability question for the Layer-0/index-view design.]**
D24: SSSOM crosswalk generation — wikilinks → URIs, tags → SKOS concepts, frontmatter keys → predicates **[2026-06-10 annotation: UNIMPLEMENTED — no SSSOM artifact or generator exists anywhere in the tree. The mapping *function* was absorbed by the projection pipeline (D58/D71) + JSON-LD context (D79); annotate vestigial.]**
...
D31: `.meta` sidecars as source of truth — replaces YAML frontmatter as authoritative metadata. File-backed: `.meta` is source; graph-backed (D43): named graph is source, `.meta` is projected view **[2026-06-10 annotation: SUPERSEDED-IN-EFFECT by D109 — authority is layer-partitioned: markdown is the authoritative *authoring* surface (L3, incl. prose), `.meta` the authoritative *queryable/interop* representation (L1–L2); neither is globally "the" source of truth.]**
...
D33: Agent-first, self-describing pod — agent discovers memory architecture via WebID → Type Index → VoID → SHACL → SPARQL. No `.claude/` injection **[2026-06-10 SCOPE AMENDMENT: the strong reading is falsified by the RQ-Salience-1 bootstrap test (cold agents don't bootstrap from self-description, 0/3; disposition can't be *installed* by the Pod, only made consumable). D33 stands for runtime *navigation* (H0 validates it); the disposition/consume-first channel is a co-equal Layer-2 delivery surface per D69 (skill/MCP). Formal channel decision pends the MCP-gateway design.]**
...
D54: Agentic-memory view — declarative policy at Pod (typed-edge vocab, Fano bound, lifecycle schema, container conventions); procedural algorithm at agent (ByteRover-style tiers, xMemory submodular selection) **[2026-06-10 annotation: boundary MOVED by D112 — curation *equipment* (signals, ledger, policy-as-data, the prov:Plan descriptor) now lives as Pod state; only the executing reasoning remains agent-side. The declarative/procedural split survives, drawn one level deeper.]**
...
D72: Compile-once principle as substrate guarantee — the substrate maintains compiled, cross-referenced state (`.meta` triples projected from body; Type Index updated on resource creation; SHACL shape catalog cached); agents don't re-derive at query time. Karpathy's "stop re-deriving, start compiling" framing applied as L2 contract. Already true in practice via the importer pipeline; D72 elevates it to a stated substrate guarantee. **[2026-06-10 annotation: the apparent tension with D80 (derive-on-demand, no materialization) resolves to a de-facto rule, now stated: MATERIALIZE what agents read on every resource access or that write-time validation needs (`.meta` projections, derived catalogs/indexes — D111 IdCatalogStore is the model); keep VIRTUAL what is request-shaped or scale-cheap (cross-cutting views, hub derivation — D113 `sub:realization sub:Virtual`). The index-view design (PD audit 2026-06-10) must pick per this rule, with materialize-on-write preferred once a view is on the read path.]**
...
D80: **Substrate-derived navigation classes** — `wiki:Hub` and breadcrumb chains are computed by Comunica CONSTRUCT views (D45 pattern), declared as `wiki:DerivedClassAffordance` / `wiki:DerivedNavigationAffordance` in the affordance catalog. Agent invocation pattern: on-demand for v1 — when agent needs hub info, runs the CONSTRUCT against `/sparql`. No materialization, no push, no D74 trigger. Materialize-then-push deferred to Rung 1.5+ once eval shows latency matters. REVISES D77's `vault:isMOC` predicate. **[2026-06-12 annotation (SP2-T6): re-cut. The original invocable CONSTRUCT/SELECT affordance design (agents run a handed query) was a dead surface — generalization probe + SP1 evals showed agents never ran handed queries unprompted; the CONSTRUCT queries were redundant with the served derived views (D113 ViewAssembler, D115 index.md). Descriptors revised to POINTERS: hub/breadcrumb descriptors now carry served-view-pointing `sh:agentInstruction` prose (hub: read the container's derived index.md + traverse `skos:broader`/`narrower`; breadcrumb: follow the broader chain on `<#this>` via `.meta` reads) plus `rdfs:seeAlso </vault/meta/views/container-index>`. Descriptor identity (`sub:DerivedClassAffordance`/`sub:DerivedNavigationAffordance`) and typing are preserved; queries removed. This aligns D80 with D72's materialize-on-write rule: the index view IS the materialized hub entry point, served as a named resource (not assembled on demand by the agent).]**

D81: **Predicate-level governance (Model A)** — SHACL shape declares which predicates the substrate governs. Listener owns triples where (subject = this resource) AND (predicate ∈ governed-set); agent owns everything else. **[2026-06-10 SOURCE-OF-TRUTH ANNOTATION: operationally the governed sets live in TS maps (`governedPredicates.ts`) — the runtime is the data-model truth. The agent-facing contract is held in sync by an existing chain (fragility-audit R-T7): TS maps → `maps.json` build sidecar → affordance descriptor `sub:governs` → asserted equal by `tests/test_markdown_projection_descriptor.py`. "Shape declares" = declares-to-agents: the SHACL shapes' `sh:agentInstruction` prose lists are documentation, hand-synced, NOT covered by the agreement test — that prose↔maps gap is the residual drift surface (FOLLOWUPS). Deriving everything from one declaration remains the cleaner future fix.]** On body write: DELETE governed-predicates, INSERT projection, leave non-governed alone. Sidesteps reification (no named graphs, no RDF-star, no per-triple prov tags). SHACL shapes stay `sh:closed false`; each shape documents its governed set via `sh:agentInstruction`. **Known limitation (RQ-Listener-1)**: CSS `FileDataAccessor.writeMetadataFile()` overwrites the `.meta` file completely on every resource PUT, before the MonitoringStore event fires. So agent enrichment via direct PATCH to `.meta` is lost when the body is rewritten. Mitigation paths for Rung 1.5: (a) read pre-write `.meta` state from Memento/git history before projection; (b) separate `.meta.agent` sidecar that CSS never touches; (c) intercept the PUT at the store layer (PassthroughStore pattern) so projection runs before CSS clears .meta. Unit tests validate Model A logic; integration test marked xfail with this diagnosis. **[2026-06-13 annotation — D82 RESOLVED BY DISSOLUTION (D116): the `.meta.agent` sidecar (mitigation path (b)) is UNBUILT, dissolved not deferred.** Provenance-scoped projection (D116) makes agent `.meta` enrichment survive body rewrites *by construction* — the floor snapshots `.meta` pre-commit and the projection subtracts exactly `f(body_old)` rather than stripping by predicate, so anything an agent wrote (incl. governed predicates on other subjects) is preserved. The real root cause was not listener timing but CSS `writeMetadataFile` clobbering on resource PUT, addressed by reading the old body pre-overwrite (floor path) or from Memento (listener backstop) — i.e. mitigation paths (a)+(c) fused, not (b). `test_agent_enrichment_survives_body_rewrite` flipped strict-xfail → expected-pass (PSP-T3b). Spec `docs/superpowers/specs/2026-06-12-provenance-scoped-projection-replacement.md`; plan `docs/superpowers/plans/2026-06-12-provenance-scoped-projection.md`; see D116.]**
...
**[2026-06-12 annotation (SP2-T7/T8): split verdict.** The **HINT half is VALIDATED and KEPT**: H0 (2026-06-09) showed every agent dereferenced `dct:conformsTo`/`rel=profile` — PROF class profiles as hints are earned. `Link: rel="profile"`, PROF descriptors, `dct:conformsTo` derivation all remain. The **SELECTION half (`?_profile=` negotiation aspiration) is RETIRED** per the configured-client fork decided in SP2-T7/T8 (2026-06-12): the hypothesized configured-client class never arrived — SAI's Authorization Agent runtime is grant-mediated LDP; LWS standardizes neither conneg-by-profile nor `Accept-Profile`; H0/H1/E8 showed the selection layer is unreachable even for capable agents. The strip-back removes `?_profile=alt` and the selection semantics; `?_profile=fused` is retained as an aggregation affordance (body+graph union in one request), not a selection. Re-entry path if a configured-client class ever arrives: add a derived view at a named URL (ViewAssembler) + a profile *hint* — both of which this Pod keeps. See `docs/research/2026-06-12-solid-design-intent-harmonization.md` §4b. `?_profile=alt` removed by SP2-T7.]**
...
**Why no MCP**: the Pod's HTTP surface (LDP + WAC + OSLC Query + Solid Notifications) is already the agent's interface contract. MCP would add a second protocol with zero capability gain for this deployment. Comunica becomes an in-process library import inside each skill. **[2026-06-10 AMENDMENT NOTE: correct on the capability axis (H0 confirms agents walk HTTP natively) but the RQ-Salience-1 consumption-leak finding (bootstrap 0/3) reopens MCP on a different axis — forced consumption + disposition installation at connect time (the MCP `instructions` initialization slot), which the open HTTP surface structurally cannot do. Formal reversal/scoping pends the MCP-gateway decision; this note prevents D91 being read as settling that question.]**
...
**Implementation note**: the listener emits two N3 Patch envelopes per write (one `DELETE` + `INSERT` pair per subject). If the body declares no typed wikilinks, the `<#this>` envelope degenerates to a no-op `DELETE { } INSERT { }`. **[2026-06-10 MECHANISM ANNOTATION: the shipped mechanism is a subject-scoped Store merge (`metaWriter.ts replaceGoverned()` — filter governed predicates per subject, merge, write), NOT N3 Patch envelopes; `buildTwoSubjectPatch()` was dead code (removed 2026-06-10). The governance INTENT (disjoint subject-scoped sets; agent-owned triples survive) is implemented and tested; only this paragraph's mechanism description was stale.]**
...
## View layer (D113, 2026-06-07)

### D113 — View layer: per-resource PROF views + conditional trailer + Person cross-cutting demonstrator (2026-06-07)

**Decision:** The deferred D107 §6 view layer is now built, realizing the neurosymbolic spec §4.3 five-tuple as Pod data and closing the second open piece of RQ-Substrate-4. **A′ default GET:** the stored body is returned byte-identical; a conditional `<!-- pod:notice -->` trailer region is appended ONLY when open curation state exists on the resource (i.e., `mem:hasOpenAction` back-pointer present in `.meta`) — `TrailerDecoratingStore` sits above `MonitoringStore` so internal reads bypass it; the D112 Probe-2 finding (Link header never reached curl agents) is directly addressed by moving the signal into the body. A **422 marker guard** in the existing `AdmissionFloorStore` (both `setRepresentation` and `addResource` paths) rejects any inbound write whose body contains the `pod:notice` marker string — no silent strip, every violation is a teaching 422. **Per-resource view conneg:** four views negotiated via `?_profile={token}` (conneg-by-profile QSA, W3C Conneg-by-Profile WD + RFC 6906): `doc` (writable document view, same as default GET plus trailer machinery), `fused` (read-only body+governed-graph union — the Tier-2/3 read backend; lens law §2.2 rules this out as default), `graph` (read-only Turtle graph-only projection), `alt` (introspection listing all negotiable profiles). The `Link: rel="profile"` MetadataWriter (previously deferred from D86) is now emitted by the existing `profile-link/` extension via `dct:conformsTo` in `.meta` — the extension already existed; the missing piece was installing the PROF profiles, now done. **Six class profiles + four view descriptors** are overlay-installed via two new overlay manifest predicates: `overlay:installsProfile` (uploads Turtle to `/vault/meta/profiles/`) and `overlay:installsView` + `overlay:installsViewArtifact` (uploads view descriptors + their SPARQL artifact files to `/vault/meta/views/`); `dct:conformsTo` is derived on write by the projection pipeline, closing the D86 FOLLOWUP. **Declared-query, engine-executed (spec §3.1):** view descriptors carry `prof:hasResource [ prof:hasRole role:mapping; dct:format "application/sparql-query" ]` CONSTRUCT artifacts; the `ViewAssembler` runs them via embedded `@comunica/query-sparql` over explicit sources — D3/D29 intact (no SPARQL HTTP endpoint); same queries are runnable by the planned Pod MCP (jeswr/solid-mcp seed) and the `solid-pod sparql` CLI; no hand-mirrored assembly (anti-drift). **Lens law as data:** `sub:writable false` (views are read-only, 405 on write), `sub:realization sub:Virtual` (assembled per request), `sub:servesAt` (declares the URL space a cross-cutting view serves). New terms minted in `overlays/wiki-memory/vocabulary/substrate.ttl` (`sub:View`, `sub:realization`, `sub:Virtual`, `sub:Materialized`, `sub:writable`, `sub:servesAt`). **Person cross-cutting demonstrator** at `/vault/views/people/` (`ViewSpaceHttpHandler`): enumerated from the Type Index, bridges `/vault/wiki/people/*.md` SKOS/schema persons to `/vault/contacts/*.ttl` vCard contacts via `schema:sameAs` — the Verborgh contacts-conundrum existence proof; one entity, two homes, read-only unified view.

**Implementation:** `css/extensions/view-layer/` — `ViewHttpHandler` (QSA interception before LdpHandler; modeled on WikiSearchHttpHandler), `ViewSpaceHttpHandler` (`/vault/views/people/`), `TrailerDecoratingStore` (PassthroughStore above MonitoringStore), `ViewAssembler` (Comunica CONSTRUCT over explicit sources), `trailer.ts` (rendering + marker constants); `AdmissionFloorStore` marker guard in `shape-validator/`; overlay machinery in `scripts/overlay/common.py` + `apply.py`; Components.js wiring in `css/config/view-layer.json`. Spec: `docs/superpowers/specs/2026-06-07-view-layer-design.md`; plan: `docs/superpowers/plans/2026-06-07-view-layer.md`.

**Status:** **BUILT + MERGED to main 2026-06-07** (merge commit `9dd3d92`; 41/41 extension unit tests + 12/12 live e2e; `make audit` 0 ERROR). **See also:** D107 (URI re-layering — the namespace slice this completes), D108 (admission floor whose marker guard is extended here), D111 (identifier-scheme substrate), D112 (curation loop whose read-path Probe-2 negative drove the trailer design), D86 (profile-link MetadataWriter now emitting via installsProfile wiring), D3/D29 (SPARQL is client-side — engine-executed declared-query honors this).

**[2026-06-12 annotation (SP2-T7): `?_profile=alt` removed.** SP2-T7 stripped the `alt` introspection view as part of the configured-client retirement (D86 §4b). The `ViewHttpHandler` no longer handles `?_profile=alt`; any request with that token falls through to the default GET. All other D113 apparatus (PROF profiles, ViewAssembler, Person demonstrator, `sub:` vocabulary, overlay manifest predicates) unchanged. Fused 404-not-500 and `ResponseMetadata` filtering fixed in the same commit. Prefixed serialization (Turtle with `sub:`/`wiki:` prefix declarations) also fixed SP2-T7. See SP2-T7 commit `641b5d2`.]**

**⚠ AMENDED 2026-06-07 (read-path cold probe + architecture decision):** the conditional trailer was the headline read-path fix here, and the cold-probe re-run did **not** validate it. Raw-trajectory audit (report `docs/plans/2026-06-07-view-layer-cold-probe-report.md`): both Arm-A agents used `curl -v` and saw the `mem:hasOpenAction` Link header too, so the trailer was never isolated as the delivery channel; the clean `curl -s`-only test was never run. The decisive follow-up — `solid-pod read` (CLI fused read) surfaces the open action on the Turtle `/id/schemes/orcid` record, content-type-agnostic — established that **governed-context delivery is a tool-tier property.** **Architecture decision (Chuck): curl = degraded follow-your-nose floor; governed metadata lives in `.meta`; the fused-read tier (CLI / planned Pod MCP) is the delivery contract.** Consequences for D113: **(a)** the A′ `TrailerDecoratingStore` is a candidate for removal/demotion — it smears a `.meta` rendering into the content body (contradicts metadata-in-`.meta`), is unproven, duplicates the fused read, and is markdown-only; floor signposts are the standard `Link rel="…hasOpenAction"` + `describedby`. **(b)** the server `?_profile=` view layer is `/vault`-scoped (no `/id/` coverage) and must be made substrate-wide + content-type-agnostic to match the CLI before the "view layer" piece of **RQ-Substrate-4 is genuinely closed — it is NOT closed yet** (the prior "closeable" claim is withdrawn). **(c)** the eval was curl-only — it measured the wrong tier; a CLI/MCP Tier-3 arm is needed. The PROF views, `sub:` vocabulary, declared-query/engine-executed pattern, Person demonstrator, and overlay machinery all stand; the read-path *delivery* design is what's reopened. **Closes FOLLOWUPS:** D86 `Link: rel="profile"` MetadataWriter + `overlay:installsProfile`. **Reopens:** trailer disposition; substrate-wide content-type-agnostic fused view; Tier-3 eval arm. **Superseded by D114** for the read-path delivery design.

### D114 — Read-path view authority: fused read as the contract, trailer removed, dual-view authority made legible (2026-06-07)

**Decision:** Corrects D113's read-path delivery. Spec: `docs/superpowers/specs/2026-06-07-read-path-view-authority-design.md`. Grounded in the agent's dual-view decision problem: every resource has a **document view** (markdown body — authoritative for prose + navigational/locally-authorable links) and a **graph view** (`.meta`/SPARQL — authoritative for typed edges, governed context, derived triples). The load-bearing finding (RQ-View-2): **the body's in-band wikilinks are a lossy, possibly-stale projection of the authoritative graph, and a document-reading agent cannot tell — so it over-trusts the body.** Three moves: **(1) Remove the trailer.** Delete `TrailerDecoratingStore` + its store-override wiring + the `AdmissionFloorStore` `<!-- pod:notice` 422 guard + the `?_profile=doc` view (collapses into the now-always-pristine default GET). The trailer was a markdown-only, single-signal shadow of the fused read at the degraded floor tier; it contradicted metadata-in-`.meta`, was unproven, and patched one symptom of body-graph-non-authoritativeness. **(2) The fused read is the contract surface.** `?_profile=fused` becomes substrate-wide + content-type-agnostic (fix the `/vault`-scoping + `.meta` merge so a fused GET on `/id/schemes/orcid` carries `mem:hasOpenAction`); it is the canonical one-request fused-read affordance usable at every tier — a curl floor agent opts in with `curl …?_profile=fused`, the CLI/MCP `read` wrap the same contract (client-side describedby-merge is an equivalent impl; add an agreement check). `?_profile=graph` is dropped (redundant with `describedby`); `?_profile=alt` introspection stays. The fused read dissolves the read crossover (agent gets body + **authoritative** graph in one representation, can't over-trust the body) and matches RQ-View-2 Tier-3 (agents used fused read, never `sparql`; SPARQL is for cross-resource aggregate only). **(3) Declare view authority (the missing piece behind "stumbling along").** A per-resource-kind, teach-the-convention artifact (PROF profile `sh:agentInstruction` + storage-description link, met on arrival via the D55 Tier-2 path) stating: which surface is authoritative for what; **read `?_profile=fused` when your question is about the graph**; author in markdown, propose graph-global judgment to the operations ledger (D109 derive/floor/loop). No per-resource in-band signal (agents won't pay the discovery cost) — stated once, applied everywhere. **Floor signposts unchanged** (already emitted, content-type-agnostic): `describedby`, `Link rel="…hasOpenAction"`, `Link rel="profile"`. **Keeps from D113:** PROF class profiles, the Person cross-cutting demonstrator, the `ViewAssembler` declared-query engine, the `sub:` vocabulary, the write grammar/floor/ledger. **Eval (proof in the pudding, post-build):** Tier-3 fused-read arm; an over-trust probe (stale in-band link vs authoritative graph — does a contract-taught agent fuse-read before answering?); floor-honesty probe. **See also:** D113 (the build this corrects), D109 (derive/floor/loop), D107 (view-layer origin), D55 (three-tier access), D3/D29 (SPARQL client-side), RQ-View-2 + D112 (the findings), Verborgh *What's in a Pod?*. **Status: APPROVED + IN BUILD 2026-06-07.**

**[2026-06-12 annotation (SP2-T7/T8): three corrections applied.** (1) **`?_profile=alt` removed** (SP2-T7 — D86 configured-client retirement; see D113 annotation). (2) **view-authority-as-PROF-artifact reversed (SP2-T8)**: D114's "move-3" declared the view-authority contract via a PROF profile `sh:agentInstruction` + a `sub:viewAuthority` pointer in `.well-known/solid`. SP2-T8 folded that content directly into the lean Layer-0 immediate literal (`.well-known/solid` `sh:agentInstruction`) — Cut-A's disposition sentences from RQ-Salience-1 preserved byte-identical; the PROF-artifact indirection removed as unnecessary machinery the harmonization confirmed was unreachable (the configured-client that would have navigated it never arrives). The contract is now inline at the cold-discovery point, not behind a pointer. (3) **`?_profile=fused` 404-not-500** and **`ResponseMetadata` filtering** fixed SP2-T7 — the fused view was returning 500 on unknown resources and leaking internal metadata into the serialized graph; both corrected. `?_profile=fused` remains the contract surface for D114's "fused read is the contract" commitment.]**

## SP2: the consumable pod (D115, 2026-06-12)

### D115 — SP2: surface + materialize the declared An layer; consumable pod (2026-06-12)

**Status:** BUILT 2026-06-12 (branch `sp2-consumable-pod`, Tasks 1–9; probes in Tasks 12–13). Spec: `docs/superpowers/specs/2026-06-10-agentic-progressive-disclosure-contract-design.md` §8; harmonization: `docs/research/2026-06-12-solid-design-intent-harmonization.md`; SP2 plan: `docs/superpowers/plans/` (SP2 task list).

**Decision:** Records four interlocking moves that surface and materialize the already-declared An (Application/ShapeTree) layer, making the Pod consumption-ready for the SP1 `pod-navigate` skill.

**Move 1 — SAI owner-side surfacing (spec §3; SP2-T1):** `hasRegistrySet` is now discoverable from the dereferenced WebID card — an agent navigating cold can reach the full `ShapeTreeRegistrySet` → `ApplicationRegistration`s in one follow-your-nose hop. The registry covers all three installed apps: wiki-memory, id-schemes, and addressbook (addressbook's `interop:Application` declaration newly minted in SP2-T2). **Harmonization delta 1 (recorded):** the RegistrySet is access-controllable BY DESIGN (SAI §3) — it is An-layer application declaration, NOT Layer-0 anonymous orientation. The storage-description `agentInstruction` (Layer-0, cut-A) is the anonymous cold-start surface; `hasRegistrySet` is the per-app registration surface for authorized agents. These are two distinct disclosure layers; `.well-known/solid` does not route through the RegistrySet.

**Move 2 — st:Description audience extension (recorded deviation; SP2-T2):** Shape Trees specifies `st:Description` as human-facing ("authorization or data listings"). This Pod consumes it agent-facing: each app's `st:Description` declares the consumption shape for that app — index-shaped for wiki-memory, operation-shaped for addressbook and id-schemes — read by the SP1 `pod-navigate` skill's orient step. The namespace stays PLURAL `shapetrees#` (conformant with the Shape Trees ED); SAI's `interop.ttl` uses the singular `interop:` namespace for its own predicates, which is upstream library drift annotated in `ontology/shapetrees.ttl`. Minimal shape trees for addressbook and id-schemes were added in SP2-T3b (SAI §6.1 expects `registeredShapeTree` on each `DataRegistration`; both apps' SHACL shapes pre-existed so the trees are thin wrappers). `pod_audit` generalized from single-tree-doc discovery to multi-document discovery without weakening checks.

**Move 3 — Derived index views (RQ-Discovery-1 fork a/d; SP2-T4/T5):** On every write to a durable wiki container, `IndexViewListener` regenerates a definition-line `index.md` child (the container's own index). The index is frontmatter-typed `sub:ContainerIndex` — a newly minted term (`sub:ContainerIndex rdfs:subClassOf schema:CreativeWork`; declared in `overlays/wiki-memory/vocabulary/substrate.ttl`). This type is what admits each index past the D98 container-fallback `ConceptShape` typing — the frontmatter-wins rule means the index is never mistyped as a concept. Derivation provenance (`prov:wasDerivedFrom/wasGeneratedBy/generatedAtTime`) is merged into the index's `.meta`, replacing-not-accumulating across regenerations (spec §7: derived views are self-describing with derivation provenance). `working/` is excluded (D73 permissive staging container; no index materialized there). The container-index view descriptor declares the agreed SPARQL query; an agreement test checks the listener's extension constant matches the descriptor artifact (anti-drift). SP2-T5b fixed a timing bug: per-container chain ownership ensures the listener never drops member events under concurrent writes.

**Named follow-ons (deferred; recorded):**
- **(a) markdown-lane write contract gated on D82:** SP2 ships the write contract Turtle-lane-first, per the 2026-06-12 harmonization — every upstream Solid/SAI contract is RDF-native; the markdown-lane enrichment (agent-declared purpose + circumstances in `.meta` alongside body writes) is a D82 (`.meta.agent` sidecar) concern, GATED on D82 landing so agent enrichment survives CSS's `.meta` overwrite semantics. **[2026-06-13: D82 dissolved by D116 (provenance-scoped projection) — this gate is LIFTED; the markdown-lane write contract is now schedulable, the natural next build.]**
- **(b) prov:agent derivation gated on the security profile (Chuck 2026-06-12):** deriving a placeholder agent identity under dev-allow-all asserts provenance the substrate cannot warrant — provenance must be warrantable, not just syntactically present. The trust-envelope framing: a `prov:agent` triple is a claim about who acted; under dev-allow-all any agent can claim any WebID, so the claim is worthless. This is deferred to the sprint where auth is wired. Not a FOLLOWUP — a principled boundary.
- **(c) ViewSpaceHttpHandler 404-throw twin bug:** SP2-T7 fixed the fused-view 404-not-500 in `ViewHttpHandler`; `ViewSpaceHttpHandler` (Person cross-cutting view) has a structurally identical throw path that was not fixed in the same commit. See FOLLOWUPS.

**Cross-references:** spec §8 for the full mechanism narrative; harmonization doc for the Solid-design-intent grounding; SP2 probes in Tasks 12–13 whose reports will append here as a dated amendment when available. **See also:** D80 (hub/breadcrumb affordances re-cut to point at served views — the index.md IS the materialized hub entry), D86 (PROF hints kept; selection retired), D109 (derive/floor/loop rule that governs the index derivation), D112 (curation loop — the index provenance self-declares), D113/D114 (view-layer apparatus the index slots into), D103 (skills bootstrap — `pod-navigate` consumes the `st:Description` declared here), SP1 `pod-navigate` skill (the consume-side twin of this construct-side sprint).

## Provenance-scoped projection (D116, 2026-06-13)

### D116 — Provenance-scoped projection replacement; D82 dissolved (2026-06-13)

**Status:** BUILT + SHIPPED 2026-06-13 (branch `prov-scoped-projection`, 8 tasks PSP-T1–T8). Spec: `docs/superpowers/specs/2026-06-12-provenance-scoped-projection-replacement.md`; plan: `docs/superpowers/plans/2026-06-12-provenance-scoped-projection.md`; probe report: `docs/plans/2026-06-13-psp-enrichment-probe-report.md`; harmonization grounding: `docs/research/2026-06-12-solid-design-intent-harmonization.md` (§2 of the spec).

**Decision:** Re-projection of the markdown body into `.meta` is now **provenance-scoped** — it touches only the projection's own prior output, instead of stripping by predicate across all subjects. The replacement contract:

```
.meta_next  =  ( .meta_current  −  f(body_old) )  ∪  f(body_new)
```

where `f` is the deterministic projection. The subtraction removes exactly the triple set the projection produced last time (recomputed from the old body), so anything an agent or another substrate component wrote survives **by construction** — including triples that use governed predicates on other subjects. This replaces `MetaWriter.replaceGoverned`'s predicate-keyed strip (`metaWriter.ts:64-69`), which conflated two jobs: idempotent regeneration (its real job) and ownership enforcement (not its job — the admission floor D108 already enforces who may write governed pairs, with 422s + teaching).

**Three-station partition** (each keeps exactly one job): **admission floor** (D108) = WHO may write a governed pair (unchanged); **projection replacement** (this decision) = idempotent regeneration of ONLY its own prior output; **curation lane** (D112) = contested values invalidated, never deleted. `PAGE/THING_GOVERNED_PREDICATES` stops being the clobber list — it remains the floor's validation dispatch + the agent-facing `sub:governs` declaration (D81 Model A), both unchanged. The F7 SP2-review fix (the `prov:wasGeneratedBy` subject-scope special case in `MetaWriter`, commit `8b6e816`) is **superseded** — its frame-scoped strip was a bespoke middle between LDP's predicate scope and the mainstream's provenance scope; deleted, subsumed by subtraction.

**Projector-version drift:** within a projector version, subtraction is exact (recompute). Across versions, `f_new(body_old)` may differ from what `f_old` wrote. Handled by a **version stamp** (`sub:projectorVersion` in `.meta` alongside `sub:bodyHash`, reusing D112's `hadPlan`-pinning pattern) + a **migration sweep** (a `pod_audit`-style walk that re-projects every markdown resource once per version bump; `make reset` is the dev path). On version match: exact subtraction. On mismatch: the degraded mode below, then re-stamp. (The projected-set-inventory alternative — store the full output inventory in `.meta` for exactness under all conditions — was rejected for `.meta` bloat + legibility; recorded as the fallback if drift handling proves noisy.)

**Two write paths:** the **primary in-band floor** (`AdmissionFloorStore` → `markdownBodyProjector`) runs inside `setRepresentation` before commit, so the old body is in hand (one `getRepresentation`; 404 → first write → empty subtraction) — exact, synchronous, no new storage. The **async listener backstop** (`MarkdownProjectionListener`, fires on out-of-band writes when the `sub:bodyHash` stamp mismatches) recovers `body_old` from **Memento** (the TimeGate's prior version — the git-backed versioning substrate already run; new `gitRead.ts` mirrors `memento/src/git.ts`, agreement-tested). When no prior memento exists (resource predates Memento / version store unavailable): degraded **pair-shadow** subtraction (remove only quads matching subject+predicate pairs the new projection emits — strictly narrower than the old predicate strip, never wider) **+ emit a `mem:StalenessDetected` / `mem:Materialization` curation signal** naming the resource so the lane sweeps residue. A **stamp-hash race guard** protects the listener path (the stamp is checked against the committed body hash to avoid acting on a stale version). Idempotency (`f(b) − f(b) ∪ f(b) = f(b)`) is property-tested.

**D82 RESOLVED BY DISSOLUTION:** D82 (the `.meta.agent` sidecar mitigation path (b) for RQ-Listener-1, documented under D81's known-limitation block) is **UNBUILT — dissolved, not deferred.** Agent `.meta` enrichment surviving body rewrites is now a construction property of the subtraction contract; the sidecar is unnecessary. The real root cause was not listener timing but CSS `FileDataAccessor.writeMetadataFile()` clobbering on resource PUT — addressed by snapshotting `.meta`/old-body pre-commit (floor) or from Memento (backstop), i.e. mitigation paths (a)+(c) fused. The strict-xfail `test_agent_enrichment_survives_body_rewrite` is flipped to an expected-pass normal test (PSP-T3b). **Unblocks** the markdown-lane write contract (`mem:rationale` on wiki lanes at crystallization, D73 preserved) — SP2 follow-on (a), previously gated on D82.

**Prior-art lineage** (harmonization research, spec §2): predicate-scoped server ownership is mainstream (LDP server-managed triples — our floor generalizes it; **keep**); schema-keyed clobbering is the anti-pattern (mem0/LangMem destroy authored content on re-derivation; every survivor — Graphiti, OBO/ROBOT `is_inferred`, SMW, the wiki lineage Karpathy/gbrain/ByteRover/AKBP — scopes regeneration to its own prior output); the strongest single-graph precedent (Graphiti) tags per-edge provenance and **invalidates rather than deletes** (already our curation posture); UMP v0.1 (universalmemoryprotocol.io) independently lands on non-destructive updates + a required write-time provenance contract (convergence evidence). A Solid `.meta` is deliberately a SHARED-write graph (D58/D81), so provenance-scoping *within* the unit — which subtraction provides — is the needed mechanism. Full grounding: `docs/research/2026-06-12-solid-design-intent-harmonization.md`.
...
**Tasks 7/9 (RDF-native lanes) — SUPERSEDED FRAMING + ADDRESSBOOK DE-CONFLATED (2026-06-18):** The D117 goal of "uniform write contract across all three lanes" is **superseded** — the `mem:rationale` contract is a **memory-substrate invariant** (L2 "explicit write + provenance"), not a Pod-wide rule. The correct frame, now stated in the 2026-06-18 memory-systems architecture spec (`docs/superpowers/specs/2026-06-18-pod-memory-systems-architecture-design.md`), partitions the Pod into **operational substrate** (traditional LD apps, governed by domain shapes, no memory contract) and **externalized memory** (the wiki-memory L3 profile + future episodic/semantic/personalized stores, where `mem:rationale` is the invariant). **Addressbook is DE-CONFLATED (2026-06-18, branch `addressbook-deconflation`):** `mem:rationale sh:property` has been removed from `contact-card`, `organization-card`, `membership`, `group`; `sub:WriteContractShape` is NOT injected into `/vault/contacts/`; the vcard shapes govern the addressbook, full stop. Live-validated: a bare vCard POST returns 201 (no `mem:rationale` required). `scripts/overlay/derive_constraints.py` excludes the contacts containers via the `CONTRACT_BEARING` filter (they are not memory containers). The **addressbook ShapeTree↔layout reconciliation** is **✅ DONE (2026-06-19, branch `addressbook-shapetree-reshape`)**: the tree was reshaped to the real nested `contacts/{Person,Organization,Group,Membership}/` layout (+Group/Membership, formal `rdfs:` annotations), each subcontainer's `ldp:constrainedBy` now derives from the tree (`derive_constraints.py`, vcard-only — no memory contract), four interop managers generated (`gen_managers.py` lane-parameterized), floor↔tree parity test added. Spec-gap fix (Chuck, Option 1): nesting added 4 container trees → registered the 4 **leaf** subcontainers (`reg:contacts-{person,organization,group,membership}`) and **narrowed `pod_audit.py`'s `registration-coverage`** to data-bearing container trees (grouping root exempt; wiki/id-schemes unchanged). `make verify` 0 ERROR / 1 D98 WARN (12 regs / 12 container trees); suite 477 passed. Spec/plan `docs/superpowers/{specs,plans}/2026-06-19-addressbook-shapetree-layout-reshape*.md`; completes memory-systems piece 1. **id-schemes is DE-CONFLATED — ✅ DONE (2026-06-19, Chuck: operational; branch `id-schemes-deconflation`).** Scheme records = operational reference data (a PID lookup table consulted in the tool-calling loop, parallel to vCard contacts). The `mem:rationale` write contract was removed from scheme registration (**lane A**: `scheme-record.shacl.ttl` + canonical mirror + 8 seeds + `CONTRACT_BEARING` now `{wiki-memory}` only); the **lane B** curation loop (`curation-proposal.shacl.ttl` on `/id/.operations/`, `mem:RealignAction` with HTTP evidence) is KEPT — `test_id_operations_floor.py::test_rationale_missing_422` still 422. No tree reshape (flat layout already matches). `make verify` 0 ERROR; bare scheme record → 201; suite 488 passed. **Net: `mem:rationale` is now a memory-substrate invariant on the wiki lane only** (operational lanes addressbook + id-schemes carry no contract; the curation/`.operations/` lane is its own thing). Stale `evals/` D111 planting scripts still write rationale on scheme bodies — refresh when next run. Plan `docs/superpowers/plans/2026-06-19-id-schemes-deconflation.md`; see FOLLOWUPS 🔵. **Probe caveat (Task 11):** both agents pre-satisfied by reading the shapes, so the laden-422 *convergence* path was not directly exercised (covered indirectly by `test_write_contract_e2e` / `test_admission_floor_integration`).
...
RQ-Conneg-1 (OPEN, 2026-06-08 — **the next session's FIRST experiments**, Chuck): **is the view layer over-built relative to pure Solid?** We layered PROF profiles + conneg-by-profile (`?_profile=`) + `Link rel="profile"` + `sub:View` on top of the dual view — but **Verborgh's *What's in a Pod?* uses NONE of it** (verified 2026-06-08: views = abstract URL↔graph mappings; no PROF/conneg-by-profile/Accept-Profile/Link-rel-profile/media-type-conneg; mechanism deferred), and the **D114 eval shows agents ignored all of it**. Live-tested: **CSS already serves document-vs-graph by pure HTTP media-type conneg** — `GET resource Accept: text/markdown` → body; `Accept: text/turtle`/`ld+json` → a graph (BUT only the `<>` page-level description, 10 lines); the **full governed graph** (`<#this>` prefLabel/broader/hasOpenAction, 22 lines) is the `.meta` resource via `describedby` (= Verborgh "different views = different URLs"). `?_profile=fused` is **aggregation not selection** (the one-request merge — its only real job); `?_profile=graph` was already dropped in D114 as redundant with describedby/`Accept: text/turtle`. Walkthrough: for our actual two views, conneg-by-profile distinguishes nothing that `Accept`+`describedby` don't. Three reasons it might be "failing" (distinguish — different fixes): (1) bad in-context instructions; (2) our implementation is buggy; (3) **agents can't do conneg-by-profile because they can't do plain conneg** (foundational — conneg-by-profile is built on conneg; eval shows agents conneg containers/`.meta` but default-GET documents). Experiment ladder: **H0** do agents do plain conneg/describedby at all? → **H1** does pure Solid (conneg+describedby+fused-as-tool) solve the dual view as well as the PROF stack? → **H2** does conneg-by-profile add agent-usable value once well-instructed + correctness-audited (or do we lack any multi-same-media-type-projection need)? Keep regardless: `.meta`/describedby, media-type conneg, fused-as-tool-behavior, the cross-cutting Person view (Verborgh's actual contribution). Shed only if H1 confirms: `?_profile=`/PROF descriptors/`Link rel=profile`/view-authority-as-PROF-artifact. **Do NOT pre-emptively rip out** — D114 validated/no-regression; strip-back is its own spec after experiments, must preserve the fused-merge. Full framing + experiment matrix: `docs/research/2026-06-08-solid-view-mechanism-vs-profiles.md`. Sibling of RQ-Salience-1 (H0/H1 feed it: standard `Accept` rides a model prior where `?_profile=fused` is bespoke). See D86/D107/D113/D114, RQ-View-2, RQ-Pod-4 (describedby-traversal gap).

RQ-Salience-1 (OPEN, 2026-06-08, out of the D114 eval): **delivered governed context doesn't change agent behavior.** D112 = "the signal never arrives" (Link header); D114 fixed arrival (fused read carries `mem:hasOpenAction`, content-type-agnostic, every tier, validated). RQ-Salience-1 = **"the signal arrives but never enters attention."** Eval datum: agents are *predicate-directed* ("find `skos:broader` → confirm → done") and an open action parked as a *sibling triple on the same subject* is structurally invisible to that attention — agent answered the stale value high-confidence with `hasOpenAction` literally in its `solid-pod read` output (neither over-trust arm mentions it anywhere). NOT delivery, NOT opaque-pointer-not-followed — **never-registered.** Reframe (away from "annotate the triple", which couples to `mem:` + assumes agents inspect statement annotations): **put the contestation, in STANDARD vocab (`owl:deprecated`/`dcterms:isReplacedBy`/`prov:wasRevisionOf`/`prov:invalidatedAtTime` — model priors), on the node the agent's own follow-your-nose traversal already visits** (the eval agent *already* fetched the broader target to confirm it). Coupling principle: app-specific SOURCE of judgment (curation ledger, `mem:RealignAction`) stays coupled; the SURFACED signal must be standard + on-path + no `mem:` literacy. Core tensions (undecided): node-level (followable, but defames the global target) vs statement-level (right, but unfollowable in RDF without reification/RDF-star); flag-the-fact vs refuse-to-serve-as-settled; data-layer weak salience vs token-layer linear-attention salience (the withdrawn D113 trailer exploited the latter — we may have under-killed it). Meta-question kept open: F1 substrate-honesty / F2 agent-disposition / F3 mis-modeled. Claude's lean (SUGGESTION, not decided): F1 + standard-vocab + on-path, content-type-aware. **Needs brainstorm + experiments before a fix** — full framing + experiment matrix (E1–E6) + prior art in `docs/research/2026-06-08-read-path-salience.md`. RQ-Substrate-4 read-path stays OPEN under this. See D112/D113/D114, RQ-View-2.
