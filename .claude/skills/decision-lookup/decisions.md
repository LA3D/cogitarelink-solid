...
**Status:** **BUILT 2026-06-07** on branch `view-layer` (15 commits; 41/41 extension unit tests + 12/12 live e2e incl. trailer end-to-end + sameAs bridge two-home card; `make audit` 0 ERROR). **Eval PENDING:** D112 Probe-2 read-path cold probe re-run against the conditional trailer channel — passing leaves RQ-Substrate-4 closeable (both pieces: URI/namespace slice via D107 + view layer via D113). Branch not yet merged/pushed (Chuck's call). **See also:** D107 (URI re-layering — the namespace slice this completes), D108 (admission floor whose marker guard is extended here), D111 (identifier-scheme substrate), D112 (curation loop whose read-path Probe-2 negative drove the trailer design), D86 (profile-link extension — the deferred MetadataWriter now emitting via installsProfile wiring), D3/D29 (SPARQL is client-side — the engine-executed declared-query pattern honors this). **Closes FOLLOWUPS:** D86 `Link: rel="profile"` MetadataWriter (now emitting) + `overlay:installsProfile` machinery (now wired).
