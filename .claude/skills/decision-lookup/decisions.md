...
Always loaded. Concise index of all architectural decisions (D1-D115, K1-K4). Vault is canonical source:
...
- **(a) markdown-lane write contract gated on D82:** SP2 ships the write contract Turtle-lane-first, per the 2026-06-12 harmonization — every upstream Solid/SAI contract is RDF-native; the markdown-lane enrichment (agent-declared purpose + circumstances in `.meta` alongside body writes) is a D82 (`.meta.agent` sidecar) concern, GATED on D82 landing so agent enrichment survives CSS's `.meta` overwrite semantics.
