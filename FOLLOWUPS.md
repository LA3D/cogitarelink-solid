...
- [ ] **Demote or remove `TrailerDecoratingStore`** (the A′ markdown trailer). It smears a `.meta` rendering into the content body (contradicts metadata-in-`.meta`), is unproven as a delivery channel, and duplicates the fused read. Options: remove entirely (rely on `Link rel="…hasOpenAction"` + `describedby` floor signposts + fused read contract), or demote to a pure pointer ("this resource has governed context — follow `describedby`"). Keep the 422 marker guard only if the trailer survives in some form.
- [ ] **Make the server fused/graph view substrate-wide + content-type-agnostic.** Today `?_profile=fused`/`graph` is `/vault`-scoped (no `/id/` coverage) and only the CLI's own sidecar merge delivers governed context on RDF. Bring the server view up to the CLI's behavior so the one declared projection is delivered identically through the server view, the CLI, and the MCP (spec §3.1's three execution contexts, only half-built). This is the real RQ-Substrate-4 view-layer close — **NOT yet done.**
- [ ] **Eval tested the wrong tier.** The probe was curl-only; the governed-context contract lives at the fused-read tier. Add a CLI/MCP (Tier-3) arm, like RQ-View-2, before drawing read-path conclusions.
