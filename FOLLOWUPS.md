...
- [ ] **★ Open-action interpretation / self-interpreting fused view (the live RQ-Substrate-4 read-path gap).** D114 eval finding: the fused read DELIVERS `mem:hasOpenAction` into the agent's context (both tiers) but agents don't ACT on it — `hasOpenAction → <op-url>` is an opaque pointer they don't dereference; the view-authority contract is deployed but unconsulted (agents go straight to the resource URL, never the storage desc/profile). Over-trust moved delivery→salience. Candidates: (a) inline `mem:rationale`+`mem:stalenessClass`+affected-predicate next to the governed triple it qualifies in the fused view (content-type-agnostic redo of what the trailer's rationale-inlining got right); (b) get the contract onto the agent's path; (c) teach open-action semantics (skill/agentGuide). Next read-path brainstorm. **RQ-Substrate-4 read-path stays OPEN.**
...
- [ ] **`?_profile=fused` (and the markdown path) on a MISSING base resource surfaces the thrown `NotFoundHttpError` as HTTP 500, not 404** — a handler waterfall-position issue (the handler bypasses CSS's error→HTTP converter). The 'no empty 200' honesty intent holds. Minor.
- [ ] **RDF `?_profile=fused` output includes the CSS `ResponseMetadata` named-graph** (posix:mtime/size) from the INTERNAL_QUADS `.meta` read — harmless noise in the merged graph; consider filtering server-bookkeeping triples from the fused representation.
