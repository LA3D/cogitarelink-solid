...
**Addressbook is DE-CONFLATED (2026-06-18, branch `addressbook-deconflation`; plan `docs/superpowers/plans/2026-06-18-addressbook-write-contract-deconflation.md`):** `mem:rationale` has been **removed** from the addressbook vcard shapes. The write contract is a memory-substrate invariant (L2), not Pod-wide; vCard operational data is governed by domain shapes only. Live-validated: a bare card POST returns 201. The D117 "uniform write contract across all three lanes" framing is superseded — see `docs/superpowers/specs/2026-06-18-pod-memory-systems-architecture-design.md`.

The **remaining RDF-native work** is two separate items:

**(a) Addressbook ShapeTree↔layout reshape — ✅ DONE (2026-06-19, branch `addressbook-shapetree-reshape`; spec/plan `docs/superpowers/{specs,plans}/2026-06-19-addressbook-shapetree-layout-reshape*.md`).** The tree was reshaped to the real nested `contacts/{Person,Organization,Group,Membership}/` layout (+Group/Membership trees, formal `rdfs:` annotations not inline comments); each subcontainer's `ldp:constrainedBy` now **derives from the tree** (`derive_constraints.py`, single source of truth — vcard-only, no memory contract); four interop managers generated (`gen_managers.py` lane-parameterized); floor↔tree parity test added. **Spec-gap fixed during impl (Chuck's call, Option 1):** nesting added 4 container trees → registered the 4 **leaf** subcontainers (`reg:contacts-{person,organization,group,membership}`, parallel to wiki) and **narrowed `pod_audit.py`'s `registration-coverage`** to data-bearing container trees so the grouping root is exempt (correct for any future nested tree; wiki/id-schemes unchanged). `make verify` 0 ERROR / 1 D98 WARN (12 registrations / 12 container trees); full suite 477 passed. **Original framing "no registry edit" was wrong** — registry.ttl + pod_audit.py + interop tests were edited.

**(b) id-schemes operational-vs-memory classification — ✅ DONE (2026-06-19, Chuck: operational; branch `id-schemes-deconflation`, plan `docs/superpowers/plans/2026-06-19-id-schemes-deconflation.md`).** Scheme records classified as operational reference data (a PID lookup table consulted in the tool-calling loop, parallel to vCard contacts). The `mem:rationale` write contract was removed from scheme registration (**lane A**: `scheme-record.shacl.ttl` + its canonical mirror `shapes/substrate/scheme-record.shacl.ttl` + 8 seeds + `CONTRACT_BEARING`); the **lane B** curation loop (`curation-proposal.shacl.ttl` on `/id/.operations/`, `mem:RealignAction` proposals with HTTP evidence) is KEPT — `test_id_operations_floor.py::test_rationale_missing_422` still 422. No tree reshape (id-schemes' tree already matches its flat layout). `make verify` 0 ERROR / 1 D98 WARN; bare scheme record → 201; full suite 488 passed. **Stale `evals/` D111 cold-probe planting scripts** (`evals/d112/`, `evals/salience-*`, `evals/viewlayer/`, `evals/e2e-walk/`) still write `mem:rationale` on scheme bodies — research artifacts, refresh when next run (not CI-blocking).
