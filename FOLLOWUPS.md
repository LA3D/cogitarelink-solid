...
- [ ] **Eval tested the wrong tier.** The probe was curl-only; the governed-context contract lives at the fused-read tier. Add a CLI/MCP (Tier-3) arm, like RQ-View-2, before drawing read-path conclusions. (Now the D114 eval, §5 of the D114 spec.)
- [ ] **D114 view-authority contract SHIPPED (page-profile `sh:agentInstruction` + `sub:viewAuthority` in `.well-known/solid`). Next = the D114 eval:** Tier-3 fused-read arm + over-trust probe (stale in-band link vs authoritative graph — does a contract-taught agent `?_profile=fused` before answering?) + floor-honesty probe. Spec §5.
